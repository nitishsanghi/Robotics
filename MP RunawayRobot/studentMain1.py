...
def estimate_next_pos(measurement, OTHER = None):
    ...
    if OTHER == None:
		...
		measurements = [measurement] #List of all measurements
		xy_estimate = (0,0) #Initialized states to zero
		#print xy_estimate
		xy_estimates = [xy_estimate] #List of all xy_estimates
		kinematics = [0] #Initialized stepsize, angle, and turn angle
		OTHER = (measurements, xy_estimates, kinematics) #History of parameters 
    else:
		dturn = atan2(measurement[1]-OTHER[0][-1][1], measurement[0]-OTHER[0][-1][0])
		stepsize = distance_between(measurement, OTHER[0][-1])
		#print stepsize
		ddturn = dturn - OTHER[2][-1]
		#print ddturn
		xy_estimate =(measurement[0] + stepsize*(cos(dturn+ddturn)), measurement[1] + stepsize*(sin(dturn+ddturn)))
		#print xy_estimate
		OTHER[0].append(measurement)
		OTHER[1].append(xy_estimate)
		OTHER[2].append(dturn)
		#print OTHER
    return xy_estimate, OTHER 

  # if OTHER == None:
        # OTHER = (measurement[0], measurement[1], 0, 0, 0) # prev_x, prev_y, stepsize, dturn, ddturn
        # xy_estimate = (0, 0)
    # else:
        # dturn_prev = OTHER[3]
        # prev_x = measurement[0]
        # prev_y = measurement[1]
        # dturn = atan2(measurement[1]-OTHER[1], measurement[0]-OTHER[0])
        # stepsize = distance_between((OTHER[0],OTHER[1]), measurement)
        # ddturn = dturn - dturn_prev
        # OTHER = (measurement[0], measurement[1], stepsize, dturn, ddturn)
        # xy_estimate =(prev_x + stepsize*(cos(dturn+ddturn)), prev_y + stepsize*(sin(dturn+ddturn)))
        # print OTHER
    # return xy_estimate, OTHER 
